...
func (c *SubcategoryStorage) GetIDByValue(ctx context.Context, Value, CategoryID int, IsComposite bool) (int, error) {
	var SubCatID int
	logrus.Println("Scanning from Get ID By Value")
	query, args, err := squirrel.Select("id").Where(squirrel.Eq{"subtype_value": Value, "is_composite": IsComposite, "category_id": CategoryID}).From(table).PlaceholderFormat(squirrel.Dollar).ToSql()
	if err != nil {
		return 0, err
	}
	row := c.c.QueryRow(ctx, query, args...)
...
func (c *SubcategoryStorage) GetData(ctx context.Context, SubcategoryID, CategoryID int) (string, int, error) {
	var AvailablePartialValues string
	var TargetSum int
	query, args, err := squirrel.Select("partial_values", "subtype_value").From(table).
		Where(squirrel.Eq{"id": SubcategoryID, "category_id": CategoryID}).
